  ...
  async function onSubmit(formdata: SignInFormData) {
    try {
      ...
      if (response.status === 200) {
        dispatch(
          setUserInfo({ username: formdata.username, email: formdata.email }),
        );
    ...
    <>
      <div className="max-w-full h-screen flex justify-center items-center bg-sky-200 sm:p-5 md:p-8">
        <div className="bg-white rounded-xl w-full h-full flex">
          ...
          <div
            className={`w-1/2 relative rounded-xl md:rounded-r-xl bg-white flex flex-1 md:flex flex-col py-6 px-3 md:py-5 md:px-24 ${roboto.className}`}
          >
            ...
            <form
              className="flex flex-col gap-2 mt-6 md:gap-3 md:mt-10"
              onSubmit={handleSubmit(onSubmit)}
              noValidate
            >
              <input
                {...register("username")}
                className="w-full h-9 md:h-11 rounded-md pl-4 focus:outline-sky-600 bg-sky-100 placeholder:text-gray-400 placeholder:text-sm text-sm md:placeholder:text-lg md:text-lg"
              ...
              <div className="relative">
                ...
                >
                  <img
                    src={isHidden1 ? hide.src : visible.src}
                    className=" w-4 md:w-6 absolute right-4 top-[10px]"
                  />
                </button>
              </div>
              {errors.password && (
              ...
              <button className="w-full h-9 md:h-11 rounded-md text-white bg-sky-600 mb-3 cursor-pointer text-sm md:text-lg">
                Create account
              </button>
            </form>
            ...
            >
              <div className="flex items-center justify-center gap-2 w-full h-full">
                <img
                  src={google.src}
                  className="size-4 md:size-5 tracking-tight md:tracking-normal"
                />
                <span className="text-sm md:text-base text-sky-600">
