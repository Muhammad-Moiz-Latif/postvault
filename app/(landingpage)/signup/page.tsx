            ...
            <div className="max-w-full h-screen flex justify-center items-center bg-sky-200 sm:p-5 md:p-8">
                <div className="bg-white rounded-xl w-full h-full flex">
                    ...
                    <div className={`w-1/2 relative rounded-xl md:rounded-r-xl bg-white flex flex-1 md:flex flex-col py-6 px-3 md:py-5 md:px-24 ${roboto.className}`}>
                        <h1 className='text-2xl text-center md:text-start  md:text-[40px] font-medium'>Create an account</h1>
                        <h1 className='text-xs text-center md:text-start md:text-sm mt-2'>Already have an account? <button onClick={() => router.push('login')} className='text-sky-600 hover:cursor-pointer'>Log in</button></h1>
                        ...
                        <form className='flex flex-col gap-2 mt-6 md:gap-3 md:mt-10' onSubmit={handleSubmit(onSubmit)} noValidate>
                            <input
                                {...register('username')}
                                className='w-full h-9 md:h-11 rounded-md pl-4 focus:outline-sky-600 bg-sky-100 placeholder:text-gray-400 placeholder:text-sm text-sm md:placeholder:text-lg md:text-lg' placeholder='Username' name='username' />
                            {errors.username && <p className="text-red-500 text-xs -my-2">{errors.username.message}</p>}
                            <input
                                {...register('email')}
                                className='w-full h-9 md:h-11 rounded-md pl-4 focus:outline-sky-600 bg-sky-100 placeholder:text-gray-400 placeholder:text-sm text-sm md:placeholder:text-lg md:text-lg' placeholder='Email' type='email' name='email' />
                            {errors.email && <p className="text-red-500 text-xs -my-2 ">{errors.email.message}</p>}
                            <div className='relative'>
                                <input
                                    {...register('password')}
                                    className='w-full h-9 md:h-11 rounded-md pl-4 focus:outline-sky-600 bg-sky-100 placeholder:text-gray-400 placeholder:text-sm text-sm md:placeholder:text-lg md:text-lg' placeholder='Password' type={isHidden1 ? 'password' : 'text'} name='password' />
                        ...
                        <button onClick={() => signIn("google", {
                            callbackUrl: '/home'
                        })} className='w-full h-9 md:h-11 rounded-md text-white border-sky-600 border-[1px] hover:cursor-pointer'>
