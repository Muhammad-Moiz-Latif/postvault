            ...
            <div className="max-w-full h-screen flex justify-center items-center bg-sky-200 p-8">
                <div className="bg-white rounded-xl w-full h-full flex">
                    ...
                    <div className={`w-1/2 rounded-r-xl bg-white flex flex-col py-5 px-24 ${roboto.className}`}>
                        ...
                        <form className='flex flex-col gap-3 mt-10' onSubmit={handleSubmit(onSubmition)}>
                            <input
                                {...register('email')}
                            ...
                            <div className='relative'>
                                <input
                                    {...register('password')}
                                    className='w-full h-11 rounded-md pl-4 focus:outline-sky-600 bg-sky-100 placeholder:text-gray-400' placeholder='Password' type={isHidden2 ? 'password' : 'text'} name='password' />
                        ...
                        <h1 className='text-sm mt-6 text-center'>Don't have an account? <button onClick={() => router.push('signup')} className='text-sky-600 hover:cursor-pointer'>Sign Up</button></h1>
                    </div>
                </div>
            </div>
        </>
